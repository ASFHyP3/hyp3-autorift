...
### Added
* The `crop_netcdf_prodcuts` console script entrypoint is now also available via the `__main__` module entrypoint. 
...
### Fixed
* The `crop_netcdf_products` console script entrypoint now uses `-` prefixed optional arguments instead of `+`.

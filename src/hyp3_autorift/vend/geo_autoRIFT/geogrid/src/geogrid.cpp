...
void interpolateOrbitHermite(isce3::core::Vec3 * position, isce3::core::Vec3 * velocity, const isce3::core::Orbit & orbit, double t)
{
...
{
    double startingRange = std::stoi(argv[1]);
    double dr =  std::stod(argv[2]);
    double sensingStart = std::stod(argv[3]);
    std::cout << "Sensing start: " << sensingStart << "\n";
    double prf =  std::stod(argv[4]);
    int nPixels = std::stoi(argv[5]);
    int nLines = std::stoi(argv[6]);
    double incidenceAngle = std::stod(argv[7]);
    int epsgcode = std::stoi(argv[8]);
    int chipSizeX0 = std::stoi(argv[9]);
    int gridSpacingX = std::stoi(argv[10]);
    double dt = std::stod(argv[11]);
    double xmin = std::stod(argv[12]);
    double xmax = std::stod(argv[13]);
    double ymin = std::stod(argv[14]);
    double ymax = std::stod(argv[15]);
    std::string demname = argv[16];
    std::string dhdxname = argv[17];
    std::string dhdyname = argv[18];
    std::string vxname = argv[19];
    std::string vyname = argv[20];
    std::string srxname = argv[21];
    std::string sryname = argv[22];
    std::string csminxname = argv[23];
    std::string csminyname = argv[24];
    std::string csmaxxname = argv[25];
    std::string csmaxyname = argv[26];
    std::string ssmname = argv[27];
    std::string tmids = argv[28];
    std::string itime = argv[29];
    std::string ftime = argv[30];
    std::string forbit = argv[31];
    
    //Some constants 
    double deg2rad = M_PI/180.0;

    //For now print inputs that were obtained
    std::cout << "\nIMPORTING THE RIGHT GEOGRID IN C!!!!!!!! \n";
    //std::cout << "Parametro" << argv[1] << "\n";
    std::cout << "\nRadar parameters: \n";
    ...
    std::string pixlinename = "window_location.tif";
    std::string offsetname = "window_offset.tif";
    std::string ro2vx_name = "window_rdr_off2vel_x_vec.tif";
    std::string ro2vy_name = "window_rdr_off2vel_y_vec.tif";
    std::string sfname = "window_scale_factor.tif";
    std::string searchrangename = "window_search_range.tif";
    std::string chipsizeminname = "window_chip_size_min.tif";
    std::string chipsizemaxname = "window_chip_size_max.tif";
    std::string stablesurfacemaskname = "window_stable_surface_mask.tif";
    int nodata_out = -32767;
...
//    int pCount = std::min( std::ceil((xmax - geoTrans[0])/geoTrans[1]), demXSize-1.) - pOff;
    ...
    while(std::getline(ifid, line))
    {
        ...
        if (line.find("<VZ unit") != std::string::npos)
        {
            ...
            if ((t0.secondsSinceEpoch()>=itimed.secondsSinceEpoch()) and (t0.secondsSinceEpoch()<=ftimed.secondsSinceEpoch()))
            {
                sv[nOrbit].datetime = t0;
                sv[nOrbit].position = pos;
                sv[nOrbit].velocity = vel;
                ++nOrbit;
            }
...
//    int ChipSizeX0 = 240;
    ...
    for (int ii=0; ii<lCount; ii++)
    {
        ...
        for (int jj=0; jj<pCount; jj++)
        {
            ...
            double dt_unity=182.0;
            double lower_thld = 0;
            double upper_thld = 20000;
            if (srxname != "")
            {
